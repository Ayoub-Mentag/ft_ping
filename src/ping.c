...
void verifyFlag(char *argv, char *validFlags) {
    if (strlen(argv) != 2) {
        printf("Invalid flag %s\n", argv);
        exit(1);
    }
    for (size_t i = 0; i < strlen(validFlags); i++) {
        if (argv[1] == validFlags[i]) {
            handleFlag(argv[1]);
            return ;
        }
    }
...
void verifyAddr(char *argv) {
    // Try to parse as raw IP first
    if (inet_pton(AF_INET, argv, &dest_addr) <= 0) {
        // If not a valid IP, try hostname resolution
